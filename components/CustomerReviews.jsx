...
export default function CustomerReviews() {
    ...
    const NextArrow = ({onClick, className}) => (
        <button
        onClick={onClick}
        className="bg-red-700 absolute -bottom-16 left-1/2 text-3xl translate-x-1 text-white">
        <MdChevronRight/>
        </button>
    )
    ...
    const PrevArrow = ({onClick, className}) => (
        <button
        onClick={onClick}
        className="bg-red-700 absolute -bottom-16 right-1/2 -translate-x-1 text-white text-3xl">  
        <MdChevronLeft/>
        </button>
    )
